...
The syntax for using string templates (or text block templates), is quite similar to using a regular string, but with a twist. Using a string template requires a processor followed by a `.` operator and then the string or text block template. Any statements in the template are denoted by a sequence starting with the new `\{` escape sequence and ending with the matching `}`. To put this all together, using a string template ends up looking something like this:

[source,java,indent=0,icons=font]
----
...
`StringTemplate.STR` is a simple processor that does "plain" string interpolation. It inserts any values directly into the string, without performing any other processing.
